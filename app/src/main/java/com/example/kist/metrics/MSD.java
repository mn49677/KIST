...
/**
 * MSD -- Minimum String Distance -- a class to generate various statistics related to the lexical
 * distance between two strings. Includes a main method as a demonstration.
 * <p>
 * 
 ...
 * 
 * <ul>
 * <li><a href="http:www.yorku.ca/mack/CHI01a.htm"> Measuring errors in text entry tasks: An
 * application of the Levenshtein string distance statistic, </a> by Soukoreff and MacKenzie (<i>CHI
 * 2002</i>). This is the paper that first introduced the minimum string distance (MSD) method for
 * calculating error rates in text entry tasks.
 * 
 * <li><a href="http:www.yorku.ca/mack/nordichi2002-shortpaper.html"> A character-level error
 * analysis technique for evaluating text entry methods </a>, by MacKenzie and Soukoreff
 * (<i>NordiCHI 2002</i>). This paper introduced the use of the "mean alignment string length" as
 * the appropropriate denominator in computing the error rates. It also demonstrates how to use the
 * "error rate matrix" (aka "confusion matrix") in analysing the types of errors that occurred.
 * </ul>
 * <p>
 * 
 * Example dialogue:
...
public class MSD
{
	 ...
	 * 
	 * The number of rows in the matrix is <code>s1.length()&nbsp;+&nbsp;1</code>. The number of
	 * columns is <code>s2.length()&nbsp;+&nbsp;1</code>. The value of the minimum string distance
	 * statistic may be retrieved from <code>msdMatrix[s1.length()][s2.length()]</code>.
	 * <p>
	 * 
	 ...
	 * 
	 * The minimum distance is the minimum number of primitive operations that can be applied to one
	 * string to yield the other. The primitives are insert, delete, and substitute.
	 * <p>
	 * 
	 ...
	 * 
	 * The error rate is computed by dividing the MSD statistic by the larger of the lengths of the
	 * presented text string and the transcribed text string, and multiplying by 100.
	 */
	 ...
	 * 
	 * 2 - A character (either c1 or c2) is added to the ends of all of the first array strings, as
	 * they are copied into the return value array. The character c1 is added to all of the
	 * presented texts, and c2 to all of the transribed texts.
	 * 
	...
	/*
	 * This function does the work of producing the alignment strings. It calls by itself
	 * recursively. The idea is to traverse the 'D' matrix, from bottom right to top left, recursing
	 * anywhere there is more than one path through the matrix.<p>
	 * 
	 * This function uses the <code>DoubleConcat()</code> function above, and the
	 * <code>StringPair</code> class.
	 */
	 ...
	 * 
	 * The alignment strings provide a convenient human-readable way to explain what transformations
	 * (insert, delete, substitute) are employed by the MSD algorithm. It's sort of an explanation
	 * of the 'D' matrix.
	 * <p>
	 * 
	 ...
	 * 
	 * The originally proposed MSD error rate was computed by dividing the MSD statistic by the
	 * larger of the sizes of the presented and transcribed text strings. As it turns out, this
	 * value differs slightly from the error rate calculated using our alignment-based error rate
	 * measure. This new-and-improved error rate measure fixes this problem. It is computed by
	 * dividing the MSD statistic by the mean size of the alignment strings.
	 */
